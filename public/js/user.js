...
(function ($) {
  ...
  addButton.on("click", async (event) => {
    ...
    const userId = $(".logged-in-user-id").get(0).innerHTML;
    if (!userId) {
      document.location.replace("/login");
    } else {
      const response = await fetch("/api/user", {
        method: "PATCH",
        body: JSON.stringify({ id1: userId, id2: id, isAdd: true }),
  ...
  delButton.on("click", async (event) => {
    ...
    if (!userId) {
      ...
      const response = await fetch("/api/user", {
        method: "PATCH",
        body: JSON.stringify({ id1: userId, id2: id, isAdd: false }),
        headers: { "Content-Type": "application/json" },
      });
      if (response.ok) {
        Swal.fire({
    ...
    <div class="buttons is-pulled-right">
      ...
      update.on("click", async function (event) {
        ...
        try {
          ...
          const avatar = helper.checkImage($(".input-avatar").get(0).files[0]);
